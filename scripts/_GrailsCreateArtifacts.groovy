...
createArtifact = { Map args = [:] ->
	...
	def classPkg
	if (type == "JavaController" && pkg) {
		classPkg = pkg
		def pos2 = pkg.lastIndexOf('.')
		if (pos2 != -1) {
			pkg = pkg[0..<pos2]
			pkg += ".controller"
		}
	...
	artifactFile = "${basedir}/${artifactPath}/${pkgPath}${className}${suffix}.${type ==~ /Java.*/ ? 'java' : 'groovy'}"
...
//    ant.copy(file: templateFile, tofile: artifactFile, overwrite: true)
    ...
    if (pkg) {
        ant.replace(file: artifactFile, token: "@artifact.package@", value: "package ${pkg}${type ==~ /Java.*/ ? ';' : ''}\n\n")
    }
    else {
	...
	ant.replace(file: artifactFile, token: "@artifact.propertyName@", value: propertyName)
	ant.replace(file: artifactFile, token: "@artifact.className@", value: className)
	ant.replace(file: artifactFile, token: "@artifact.classPkg@", value: classPkg)
